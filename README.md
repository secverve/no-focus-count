...
- 숫자·포인트·패널 색, 운영체제 공통 전체 표시 투명도, 표시 글꼴, 블러, 크기, 글로우 테마 설정
- 상단 집중 시간·모니터 설정과 목표 체크리스트, 완료 체크·삭제·재시작·내보내기·동기화 유지
- 세션별 집중/이탈 시간, 이탈 원인, 앱, 창 제목과 동의한 URL을 “집중 → YouTube 3분 → Codex 40초” 이동 타임라인으로 기록
...
집중 세션 중 No Focus Count나 설정 창을 포함한 다른 창을 열면 예외 없이 이탈로 처리합니다. Linux Wayland는 다른 앱의 활성 창 정보를 제한하므로 커서가 위치한 모니터를 기준으로만 판정합니다. Linux에서 창과 탭까지 감지하려면 X11 세션을 사용하세요.
...
처음 실행할 때 **시스템 설정 → 개인정보 보호 및 보안 → 손쉬운 사용** 권한이 필요합니다. 상단의 창 선택 버튼이나 설정의 `ALT-TAB 창 선택`을 누르면 별도 선택창이 열립니다. 모니터를 먼저 고른 뒤 실제 창 미리보기·앱 아이콘·창 제목·창 번호를 보며 Tab/화살표와 Enter로 선택할 수 있습니다. 선택 창이 다른 모니터로 이동하면 타이머가 즉시 멈추고 이동 기록에 남습니다. URL 기록은 브라우저 자동화 권한, 미리보기와 Focus Receipt는 화면 및 시스템 오디오 기록 권한을 추가로 요청할 수 있습니다. Focus Receipt는 기본적으로 꺼져 있고, 켠 경우에도 이탈이 2초 이상 이어진 시점의 해당 창 한 장만 480px JPEG로 로컬 저장합니다.
